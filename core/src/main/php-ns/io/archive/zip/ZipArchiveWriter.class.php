 ...
 */
 ...
 use 
  ...
  class ZipArchiveWriter extends \lang\Object {
     ...
     */
    public function __construct(\io\streams\OutputStream $stream, $unicode= FALSE) {
      $this->stream= $stream;
    ...
    public function addDir(ZipDirEntry $entry) {
      if (strlen($entry->getName()) > 0xFFFF) {
        throw new \lang\IllegalArgumentException('Filename too long ('.$nameLength.')');
      }
     ...
     * @return  int
     */
    protected function dosTime(\util\Date $date) {
      return 
     ...
     * @return  int
     */
    protected function dosDate(\util\Date $date) {
      return
