 ...
 */
 ...
 use text\doclet\VarTag, text\doclet\Taglet;
   ...
   * @purpose  Taglet
   */
  class VarTaglet extends \lang\Object implements Taglet {
    ...
    public function tagFrom($holder, $kind, $text) {
      for ($parse= $text.' ', $i= 0, $s= strlen($parse), $brackets= 0; $i < $s; $i++) {
        if (' ' === $parse{$i} && 0 === $brackets) {
          ...
          return new VarTag(substr($parse, 0, $i), $m[0], isset($m[1]) ? $m[1] : '');
        } else if ('<' === $parse{$i}) {
          $brackets++;
        } else if ('>' === $parse{$i}) {
          $brackets--;
      ...
      return new VarTag('void', 'arg', $text);
    }
  } 
?>
