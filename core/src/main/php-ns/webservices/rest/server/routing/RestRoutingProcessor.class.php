 ...
 */
  ...
  namespace webservices\rest\server\routing;
 use 
   ...
   */
  class RestRoutingProcessor extends \lang\Object {
    private
      $bindings= array(),
      $dataCaster= NULL;
    ...
    protected function parseName($name) {
      if (!preg_match('/^([^\[]*)(\[(.*)\])?$/', $name, $matches)) {
        throw new \lang\IllegalArgumentException('Can not parse binding: '.$name);
      }
    ...
    public function getBinding($token) {
      ...
      if (!array_key_exists($name, $this->bindings)) {
        throw new \lang\IllegalArgumentException('No binding to inject '.$token);
      }
      ...
      if ($key !== NULL) {
        ...
        if (!array_key_exists($key, $binding)) {
          throw new \lang\IllegalArgumentException('Binding '.$name.' has no key '.$key);
        }
    ...
    public function execute(RestRoutingItem $routing, $values) {
      ...
      foreach ($routing->getArgs()->getInjections() as $i => $name) {
        ...
        try {
          ...
          throw new \lang\ClassCastException('Can not convert argument #'.$ref.' from '.$name.': '.$e->getMessage(), $e);
        }
      }
      ...
      foreach ($names as $i => $name) {
        ...
        try {
          ...
          throw new \lang\ClassCastException('Can not convert object field '.$name.': '.$e->getMessage(), $e);
        }
      }
      ksort($args);
