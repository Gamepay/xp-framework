 ...
 */
 ...
 use webservices\rest\server\routing\RestRoute;
   ...
   * @purpose Route
   */
  class RestMethodRoute extends \lang\Object implements RestRoute {
    ...
    public function process($args= array()) {
      foreach ($this->method->getParameters() as $n => $arg) {
        if (!isset($args[$n]) && !$arg->isOptional()) {
          throw new \lang\IllegalArgumentException('Argument '.$arg->getName().' missing');
        }
      }
