 ...
 */
 ...
 use 
   ...
   * @purpose Router
   */
  class RestAnnotationRouter extends \lang\Object implements RestRouter {
    protected $base= '';
        ...
        foreach ($handler->getMethods() as $method) {
          ...
          if ($method->hasAnnotation('webmethod', 'inject')) foreach ($method->getAnnotation('webmethod', 'inject') as $ref => $name) {
            if (!is_numeric($ref) && !$args->hasArgument($ref)) {
              throw new \lang\IllegalArgumentException('Argument '.$ref.' does not exist for injecting '.$name);
            }
