 ...
 */
  ...
  namespace webservices\rest\server\rpc;
 use 
    scriptlet\HttpScriptlet,
    webservices\rest\server\routing\RestRoutingProcessor,
    webservices\rest\server\transport\HttpRequestAdapterFactory,
    webservices\rest\server\transport\HttpResponseAdapterFactory,
  ...
  class RestHttpScriptlet extends HttpScriptlet {
    ...
    public function doProcess($request, $response) {
      ...
      try {
        ...
        if ($e instanceof ScriptletException) {
          ...
          $res->setStatus(\peer\http\HttpConstants::STATUS_INTERNAL_SERVER_ERROR);
        }
    ...
    private function execute($req, $res) {
      ...
      try {
        ...
        throw new \scriptlet\HttpScriptletException($e->getMessage(), \peer\http\HttpConstants::STATUS_BAD_REQUEST, $e->getCause());
      }
      ...
      if (!$routed)  throw new \lang\IllegalStateException(
    ...
    private function getRequestRoutes($req) {
      ...
      if (!$this->router->resourceExists($path)) {
        throw new \scriptlet\HttpScriptletException('Resource '.$path.' does not exist.', \peer\http\HttpConstants::STATUS_NOT_FOUND);
      }
      
      $routings= $this->router->routesFor($req, $res);     
      if (count($routings) === 0) {
        throw new \scriptlet\HttpScriptletException('The method '.$req->getMethod().' is not allowed on the resource '.$path.'.', \peer\http\HttpConstants::STATUS_METHOD_NOT_ALLOWED);
      }
