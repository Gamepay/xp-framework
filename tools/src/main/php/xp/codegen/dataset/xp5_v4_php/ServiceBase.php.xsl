...
>
  ...
  <xsl:template match="/">
    <xsl:value-of select="my:setFilename(concat('base/', my:camelCase(/document/table/@class), 'ServiceBase.class.php'))" />
 ...
 */
  ...
  uses(
    'rdbms.Criteria',
    'rdbms.Peer',
    '</xsl:text><xsl:value-of select="concat(/document/table/@package, '.base.', my:camelCase(/document/table/@class))" /><xsl:text>ServiceBaseInterface',
    '</xsl:text><xsl:value-of select="concat(/document/table/@package, '.', my:camelCase(/document/table/@class))" /><xsl:text>',
    '</xsl:text><xsl:value-of select="concat(/document/table/@package, '.', my:camelCase(/document/table/@class))" /><xsl:text>Service'
  );&#10;
  
  use \Criteria as Criteria;
  use \Peer as Peer;  
  use </xsl:text><xsl:value-of select="concat(/document/table/@namespace, '\base\', my:camelCase(/document/table/@class), 'ServiceBaseInterface', ' as ', my:camelCase(/document/table/@class), 'ServiceBaseInterface',';')"/><xsl:text>
  use </xsl:text><xsl:value-of select="concat(/document/table/@namespace, '\', my:camelCase(/document/table/@class), ' as ', my:camelCase(/document/table/@class), ';')"/><xsl:text>
  use </xsl:text><xsl:value-of select="concat(/document/table/@namespace, '\', my:camelCase(/document/table/@class), 'Service', ' as ', my:camelCase(/document/table/@class), 'Service', ';')"/><xsl:text>&#10;</xsl:text>
  ...
  abstract class </xsl:text><xsl:value-of select="my:camelCase(@class)"/><xsl:text>ServiceBase implements </xsl:text><xsl:value-of select="my:camelCase(@class)"/><xsl:text>ServiceBaseInterface {
     ...
     *
     * @return </xsl:text><xsl:value-of select="concat(/document/table/@package, '.', my:camelCase(/document/table/@class))" /><xsl:text>Service
     */
    public static function getInstance() {
      if (!isset(self::$instance)) {
        self::$instance= new </xsl:text><xsl:value-of select="my:camelCase(@class)"/><xsl:text>Service();
      }
    ...
    protected function getPeer() {
      return Peer::forName('</xsl:text><xsl:value-of select="concat(/document/table/@package, '.', my:camelCase(/document/table/@class))" /><xsl:text>');
    }
  </xsl:text>
