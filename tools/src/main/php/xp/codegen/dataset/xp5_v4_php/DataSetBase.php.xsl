 ...
 */
  ...
  use \DataSet as DataSet;
  use \FieldType as FieldType;
  use \Peer as Peer;
  use \HashmapIterator as HashmapIterator;  
  use </xsl:text><xsl:value-of select="concat(/document/table/@namespace, '\base\', my:camelCase(/document/table/@class), 'BaseInterface', ' as ', my:camelCase(/document/table/@class), 'BaseInterface', ';')"/><xsl:text>&#10;</xsl:text>
      ...
      <xsl:if test="0 &lt; count(my:referencing($this) | my:referenced($this))"><xsl:text>
        ...
        <xsl:for-each select="my:referencing($this)"><xsl:text>
          '</xsl:text><xsl:value-of select="@role" /><xsl:text>' => array(
            'classname' => '</xsl:text><xsl:value-of select="concat($package, '.', my:camelCase(my:prefixedClassName(@table)))" /><xsl:text>',
            'key'       => array(
        ...
        <xsl:for-each select="my:referenced($this)"><xsl:text>
          '</xsl:text><xsl:value-of select="@role" /><xsl:text>' => array(
            'classname' => '</xsl:text><xsl:value-of select="concat($package, '.', my:camelCase(my:prefixedClassName(../../@name)))" /><xsl:text>',
            'key'       => array(
  ...
  <!-- Create getters and setters -->
    ...
    public function set</xsl:text><xsl:value-of select="my:ucfirst(my:camelCase(@name))" /><xsl:text>(</xsl:text>$<xsl:value-of select="my:camelCase(@name)"/><xsl:text>) {
      return $this->_change('</xsl:text><xsl:value-of select="@name"/><xsl:text>', $</xsl:text><xsl:value-of select="my:camelCase(@name)"/><xsl:text>);
    }&#10;</xsl:text>
  </xsl:for-each>
  ...
  <xsl:for-each select="my:referencing($this)">
    ...
    <xsl:variable name="fullclassname"   select="concat($package, '.', my:camelCase(my:prefixedClassName(@table)))" />
    ...
    <xsl:variable name="keys4criteria">
      <xsl:for-each select="key">
        <xsl:text>          array('</xsl:text><xsl:value-of select="@sourceattribute" /><xsl:text>', $this->get</xsl:text><xsl:value-of select="my:ucfirst(my:camelCase(@attribute))" /><xsl:text>(), EQUAL)</xsl:text>
  ...
  <xsl:for-each select="my:referenced($this)">
    ...
    <xsl:variable name="keys4criteria">
      <xsl:for-each select="key">
        <xsl:text>          array('</xsl:text><xsl:value-of select="@attribute" /><xsl:text>', $this->get</xsl:text><xsl:value-of select="my:ucfirst(my:camelCase(@sourceattribute))" /><xsl:text>(), EQUAL)</xsl:text>
        <xsl:if test="position() != last()"><xsl:text>,&#10;</xsl:text></xsl:if>
      </xsl:for-each>
    </xsl:variable>
